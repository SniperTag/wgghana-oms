<div id="page-container"
    ...
    @if (session()->has('success'))
        <div class="alert alert-success mb-4 shadow rounded text-sm bg-green-100 text-green-800">
            {{ session('success') }}
        </div>
    @endif

    @if (session()->has('error'))
        <div class="alert alert-danger mb-4 shadow rounded text-sm bg-red-100 text-red-800">
            {{ session('error') }}
        </div>
    @endif
    <div>
...
{{-- Checked-in section cards --}}

 <div class="grid grid-cols-1 md:grid-cols-4 gap-4 mb-6">
                        <div class=" text-green-800 p-4 rounded shadow">
                            <h3 class="text-sm font-semibold">Approved Checked In</h3>
                            <p class="text-3xl">{{ $checkInCount }}</p>
                        </div>
                        <div class=" text-red-800 p-4 rounded shadow">
                            <h3 class="text-sm font-semibold">Declined Checked In</h3>
                            <p class="text-3xl">{{ $declinedCheckInCount }}</p>
                        </div>
                        <div class=" text-yellow-800 p-4 rounded shadow">
                            <h3 class="text-sm font-semibold">Todays Check In</h3>
                            <p class="text-3xl">{{ $todayCheckInCount }}</p>
                        </div>
                        <div class=" text-purple-800 p-4 rounded shadow">
                            <h3 class="text-sm font-semibold">Total Checkins</h3>
                            <p class="text-3xl">{{ $totalVisitorCount }}</p>
                        </div>
                    </div>
                    ...
                    @if (session()->has('message'))
                        <div
                            class="alert alert-success mb-4 shadow rounded text-sm bg-green-100 text-green-800 px-4 py-2">
                            {{ session('message') }}
                        </div>
                    @endif
                            ...
                            <table class="min-w-full bg-white border rounded shadow text-sm">
                                ...
                                <tbody>
                                    @foreach ($appointments as $appointment)
                                        <tr
                                            class="border-b @if ($appointment->status === 'rescheduled') bg-yellow-50 @elseif($appointment->status === 'pending') bg-yellow-100 @endif">
                                            ...
                                            <td class="px-4 py-2 text-blue-700">
                                                {{ $appointment->date->format('M d, Y h:i A') }}</td>
                                            <td class="px-4 py-2">
                                                <span
                                        ...
                                        @if ($selectedExpandedId === $appointment->id)
                                            <tr class="bg-gray-50">
                                                <td colspan="6" class="px-4 py-3 text-gray-700 border-t text-sm">
                                                    ...
                                                    <div class="mt-3">
                                                        <button
                                                            wire:click="showRescheduleModal({{ $appointment->id }})"
                ...
                <!--Checkin Approval Table Section-->
                @livewire('visitor.host-checkin-approvals')
            </div>
        </main>
        <!-- END Main Container -->
    </div>
